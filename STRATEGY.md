...
| 2026-06-09 | 1.2 | Выравнивание под аудит кода: §1 as-is уточнён (разрывы документ↔код), §1.3 монетизация помечена как не реализованная. Источник плана пересборки — `docs/UPGRADE_PLAN.md` |
...
| RoomScan / ARDesigner | ⚠️ Частично | Скан/AR работают **только с LiDAR**; путь без LiDAR — заглушка; AR-координаты считает LLM, не движок; каталог USDZ пуст (placeholder). См. `docs/UPGRADE_PLAN.md` |
...
> **⚠️ Реальность по аудиту (2026-06-09):** «✅ Готов» означает «код компилируется и архитектура на месте», но между обещанным сценарием и кодом есть разрывы. Главные: (1) путь без LiDAR — заглушка, де-факто только iPhone Pro; (2) на экране AR-расстановки координаты мебели считает LLM, а зрелый детерминированный движок подключён только к чат-советнику; (3) каталог USDZ пуст → рендерится placeholder; (4) монетизация (§1.3) в коде отсутствует. Полная сверка и поэтапный план закрытия — [`docs/UPGRADE_PLAN.md`](docs/UPGRADE_PLAN.md).
...
> **⚠️ Статус:** это целевая модель монетизации (канон цен), **в коде она пока не реализована** — нет ни ЮKassa, ни тарифов FREE/PRO/BUSINESS, ни пейволла (по аудиту 2026-06-09). Реализация — `docs/UPGRADE_PLAN.md`: Фаза 1 (подписка) и Фаза 3 (CPA).
