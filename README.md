...
Текущий этап (Core Layer + Backend + Тесты) разрабатывается на **Windows 11** через **Polza IDE** с моделью **DeepSeek V4 Flash**.

Для Features (AR, RoomScan, UI, Marketplace) необходим **Mac с Xcode 16+**.

Компиляция и выполнение тестов iOS — через **GitHub Actions** на `macos-14` runner.

---

## Backend

Папка `backend/` содержит Yandex Cloud Function на Node.js 20.

| Файл | Назначение |
|------|-----------|
| `index.js` | Точка входа. Разбор запроса, rate limit, triplex fallback |
...
| `package.json` | Нет внешних зависимостей |

Rate Limit: **20 запросов/минуту** на userId (in-memory).
