...
> **⚠️ Статус по аудиту кода (2026-06-09):** код компилируется и ядро на месте, но между описанным сценарием и кодом есть разрывы. Кратко: путь без LiDAR — заглушка (де-факто только iPhone Pro); на экране AR-расстановки координаты мебели считает LLM, а не детерминированный движок (он подключён только к чат-советнику); каталог USDZ пуст → рендерится placeholder; монетизации в коде нет. Полная сверка и план закрытия — [`docs/UPGRADE_PLAN.md`](docs/UPGRADE_PLAN.md).
