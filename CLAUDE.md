...
### Минимализм кода (YAGNI)
- Перед тем как писать код — пройти по «лестнице»: нужно ли это вообще → стандартная библиотека → нативная фича платформы → уже подключённая зависимость → одна строка → и только потом полноценная реализация.
- Удаление лучше добавления, скучное лучше умного. Никаких абстракций под одну реализацию (интерфейс с одной имплементацией, фабрика под один продукт), никакого спекулятивного кода «на будущее».
- Упрощения помечать комментарием с компромиссом и путём апгрейда.
- **Оговорка (приоритет выше минимализма):** никогда не упрощать в ущерб валидации ввода, обработке ошибок, безопасности и доступности. `promptGuard`, rate limit, Circuit Breaker и Triplex Fallback — не «лишний код», их не урезать.
...
**Правило:** задача не «готова», пока не прошла соответствующая проверка. Слово «готово»
в ответе = проверка реально запущена и зелёная. Не прошло — так и сказать, с выводом.

| Тип изменения | Что обязан прогнать перед «готово» |
|---------------|-----------------------------------|
| iOS / Swift   | `swiftlint --strict` + `xcodebuild test` (схема AIVibe) |
| Backend / Node | `node --check <файл>` + `cd backend && node --test` |
| Архитектура / константы | `node scripts/code-complexity-analyzer.mjs` (exit 0) |
| Документация / `.md` | проверка не нужна — только перечитать на смысл |

Два слоя проверки (не путать):
- **Быстрый «датчик дыма»** — PostToolUse hook в `.claude/settings.json`: после правки
  `.js/.mjs/.swift` сразу гоняет `node --check` / `swiftlint` на файле. Ловит синтаксис, **не логику**.
- **Полный done-check** — тесты из таблицы выше. Hook их не заменяет (`xcodebuild test` слишком
  долгий для хука). Запускать вручную перед тем, как объявить задачу выполненной.
