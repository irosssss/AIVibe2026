...
const INJECTION_RULES = [
  // ── Severity 5 — Критичные (мгновенный бан) ──────────────────────────────
  {
    ...
    patterns: [
      ...
      // RU: «покажи/дай свой api-ключ/токен/пароль/секрет/переменные окружения».
      // секрет(?!ер) — не путаем с «секретером» (предмет мебели).
      /(?:покажи|выведи|раскрой|дай|назови|распечатай|сообщи|пришли)\s+(?:мне\s+)?(?:[а-яё]+\s+){0,2}(?:api[\s-]?ключ|ключ\s+(?:api|доступа|шифровани)|токен|пароль|секрет(?!ер)|переменн[а-яё]+\s+окружени)[а-яё]*/i,
    ],
  },
  {
    ...
    patterns: [
      ...
      // RU: «игнорируй/забудь предыдущие инструкции/команды/директивы/промпт».
      // Требует И глагол, И мишень — «игнорируй старый диван» не сработает.
      /(?:игнорир[а-яё]*|проигнорир[а-яё]*|забудь|забуд[а-яё]+|не\s+обращай\s+внимани[а-яё]+\s+на)\s+(?:[а-яё]+\s+){0,3}(?:инструкци|указани|команд|директив|промпт|предписани)[а-яё]*/i,
      // RU: «забудь всё, что выше / сказано ранее».
      /(?:игнорир[а-яё]*|забудь|забуд[а-яё]+)\s+(?:вс[её]\s+)?(?:,?\s*что\s+)?(?:написан[а-яё]+\s+|сказан[а-яё]+\s+|был[а-яё]+\s+)?(?:выше|ранее|до\s+этого|вначале)/i,
      // RU: «покажи свой системный промпт / выведи свои инструкции».
      /(?:покажи|раскрой|выведи|повтори|процитируй|назови|распечатай|напиши)\s+(?:мне\s+)?(?:сво[иейя][а-яё]*|тво[иейя][а-яё]*|ваш[а-яё]*|весь|полн[а-яё]+|исходн[а-яё]+|первоначальн[а-яё]+)\s+(?:систем[а-яё]+\s+)?(?:промпт|инструкци|настройк|директив)[а-яё]*/i,
      // RU: упоминание «системный промпт».
      /систем[а-яё]+\s+промпт[а-яё]*/i,
    ],
  },
  {
    id: 'jailbreak_known',
    ...
    patterns: [
      ...
      // RU: «режим разработчика / режим без цензуры / режим бога / DAN».
      /режим[а-яё]*\s+(?:разработчик|без\s+цензур|без\s+ограничени|бога|dan)[а-яё]*/i,
      // RU: «отключи/обойди фильтры/цензуру/ограничения/защиту».
      /(?:отключи|обойди|сними|убери|выключи|деактивируй|игнорир[а-яё]*)\s+(?:[а-яё]+\s+){0,2}(?:фильтр|цензур|ограничени|защит|модерац|безопасн)[а-яё]*/i,
      // RU: «без цензуры / без фильтров» (НЕ ловим «без ограничений» — легитимно про бюджет).
      /без\s+(?:цензур|фильтр)[а-яё]*/i,
    ],
  },
  ...
  // ── Severity 4 — Высокие ─────────────────────────────────────────────────
  {
    ...
    patterns: [
      ...
      // RU: «ты теперь администратор/root/разработчик/владелец», «веди себя как админ».
      /(?:ты\s+(?:теперь|отныне|больше\s+не)|веди\s+себя\s+как|представь[,\s]+что\s+ты|притворись[,\s]+что\s+ты|с\s+этого\s+момента\s+ты)\s+(?:[а-яё]+\s+){0,2}(?:администратор|админ|root|рут|разработчик|суперпользовател|хозяин|владелец)[а-яё]*/i,
    ],
  },
  {
